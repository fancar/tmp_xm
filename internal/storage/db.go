...
var (
	...
	ErrTransactionRollback = fmt.Errorf("rollback")
)
...
func Transaction(f func(tx sqlx.Ext) error) error {
	tx, err := db.Beginx()
	if err != nil {
		return fmt.Errorf("storage: can't begin the transaction %v", err)
	}

	err = f(tx)
	if err != nil {
		if rbErr := tx.Rollback(); rbErr != nil {
			return fmt.Errorf("storage: can't rollback the transaction %v", err)
		}
	...
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("storage: can't commit the transaction %v", err)
	}
	return nil
}
